...
const AvatarIcon = ({ icon, iconStyle, size, }) => {
    ...
    return (React.createElement(Icon, { style: StyleSheet.flatten([iconStyle]), color: icon.color || 'white', name: icon.name || 'account', size: icon.size || iconSize, type: icon.type || 'material-community' }));
};
export const Avatar = ({ onPress, onLongPress, onPressIn, onPressOut, Component = onPress || onLongPress || onPressIn || onPressOut
    ? Pressable
    : View, containerStyle, icon, iconStyle, source, size = 'small', avatarStyle, rounded, title, titleStyle, overlayContainerStyle, imageProps, renderCustomContent, ImageComponent = RNImage, children, pressableProps, ...rest }) => {
    const width = typeof size === 'number' ? size : avatarSizes[size] || avatarSizes.small;
    const height = width;
    ...
    if (source) {
        componentToRender = (React.createElement(Image, { testID: "RNE__Avatar__Image", containerStyle: imageContainerStyle, source: source, PlaceholderContent: (title && (React.createElement(AvatarTitle, { title: title, titleStyle: titleStyle, size: size }))) ||
    ...
    return (React.createElement(Component, { style: StyleSheet.flatten([
            styles.container,
            { height, width },
