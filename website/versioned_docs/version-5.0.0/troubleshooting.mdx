...
Although we try to make the experience of using React Native vElements
...
1. Check your `package.json` for the version of `@rn-vui/themed` you are
   using.
...
the error comes from React Native vElements looking for
...
2. Install React Native Elements
   ...
   ```bash
   # yarn
   yarn && yarn add @rn-vui/base @rn-vui/themed
   ```
